...
describe("sessionIndex 标题清洗(命令注入回显不裸露)", () => {
  it("首条是斜杠命令 → 标题清成紧凑 /名 参数,不带 XML 标签/SGR 残骸", async () => {
    const content =
      [
        userLine(
          "2026-01-01T00:00:01.000Z",
          "<command-name>/gstack-office-hours</command-name>" +
            "<command-args>想一想还能做啥</command-args>" +
            "<local-command-stdout>Set to [1mOpus[22m</local-command-stdout>"
        ),
        asstLine("2026-01-01T00:00:02.000Z", "好的"),
      ].join("\n") + "\n";
    const p = write("cmd-title.jsonl", content);
    const idx = await getSessionIndex(p, { fileMtimeMs: 1, projectId: "proj", sessionId: "sid" });
    expect(idx.header.firstUserText).toBe("/gstack-office-hours 想一想还能做啥");
    expect(idx.header.title).toBe("/gstack-office-hours 想一想还能做啥");
    // 不裸露标签、不裸露 SGR 残骸。
    expect(idx.header.title).not.toContain("<command");
    expect(idx.header.title).not.toContain("[22m");
  });

  it("首条是机器注入样板(skill 正文)→ 清成空则跳过,标题用下一条真人消息", async () => {
    const content =
      [
        userLine("2026-01-01T00:00:01.000Z", "Base directory for this skill: /tmp/x\n# /foo"),
        userLine("2026-01-01T00:00:02.000Z", "真正的第一句人话"),
      ].join("\n") + "\n";
    const p = write("boilerplate-first.jsonl", content);
    const idx = await getSessionIndex(p, { fileMtimeMs: 1, projectId: "proj", sessionId: "sid" });
    expect(idx.header.firstUserText).toBe("真正的第一句人话");
    expect(idx.header.title).toBe("真正的第一句人话");
  });

  it("首条含 caveat + 真人正文 → caveat 剥掉、留正文", async () => {
    const content =
      [
        userLine(
          "2026-01-01T00:00:01.000Z",
    ...
    const p = write("caveat-title.jsonl", content);
    const idx = await getSessionIndex(p, { fileMtimeMs: 1, projectId: "proj", sessionId: "sid" });
    expect(idx.header.title).toBe("帮我索引 /tmp/repo");
    expect(idx.header.title).not.toContain("Caveat");
  });
});
