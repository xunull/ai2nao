...
export function Vscode() {
  ...
  const statusQ = useQuery({
    ...
    queryFn: () => apiGet<VscodeStatus>("/api/vscode/status"),
  });
  const projectsQ = useQuery({
    queryKey: ["vscode-projects", submittedQ, scope, includeMissing, offset],
    queryFn: () => apiGet<PageRes<VscodeProject>>(`/api/vscode/recent-projects?${common}`),
  });
  const entriesQ = useQuery({
    queryKey: ["vscode-entries", submittedQ, scope, includeMissing, offset],
    queryFn: () => apiGet<PageRes<VscodeEntry>>(`/api/vscode/recent?${common}`),
  });
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-xl font-semibold">VS Code 工作区</h1>
...
function StatusPanel({
  ...
  return (
    <div className="rounded border border-[var(--border)] bg-white p-4 text-sm space-y-2">
      <div>
        Code 最近记录：{status.counts.active} 个活跃 · 远程 {status.counts.remote} · 已消失{" "}
...
function ProjectsTable({
  ...
  res: PageRes<VscodeProject> | undefined;
  isLoading: boolean;
  error: unknown;
}) {
...
function EntriesTable({
  ...
  res: PageRes<VscodeEntry> | undefined;
  isLoading: boolean;
  error: unknown;
}) {
