...
常见疑问:"某一天的量是怎么拆出来的?是估算的吗?" **不是估算,是精确的。**
每天的数字都来自 rollout 里逐条事件的真实数据,没有任何按天数平均或按比例分摊。

### 4.1 每个 token 事件自带时间戳 + 累计值

rollout jsonl 里每个回合后都有一条 `token_count` 事件,带两个关键字段:
...
1. `timestamp` —— 这条事件**发生的真实时刻**(精确到秒)。
2. `total_token_usage` —— **到这一刻为止的累计总量**(只增不减)。

旧 bug 只取**最后一条**事件的累计值,挂到 session 的 `last_updated_at` 一天。
现在**每一条**事件都按它**自己的** timestamp 入库。

### 4.2 当天的量 = 相邻累计值相减

因为 `total_token_usage` 是累计的,某次事件的"当次新增" = 本次累计 − 上次累计;
把每次新增按它的时间戳归到当天,加起来就是当天的量。用那个长 session 的真实数:

```
事件时间戳(当天最后一条)   累计 total      当天 = 本次累计 − 前一天累计
─────────────────────────   ──────────      ────────────────────────────
6/14                         79,550,837
6/15                        118,331,421      6/15 = 118,331,421 − 79,550,837  ≈ 38.8M
6/16                        152,319,586      6/16 = 152,319,586 − 118,331,421 ≈ 34.0M
6/17                        196,861,354      6/17 = 196,861,354 − 152,319,586 ≈ 44.5M
6/18                        203,626,497      6/18 = 203,626,497 − 196,861,354 ≈  6.7M
```

差值大小完全由"那天真实发生了多少 token_count 事件"决定:用得多差值就大,
没用差值就是 0。**唯一的"减法"是把累计值还原成当次增量,这是无损的**,不是猜。

### 4.3 怎么证明不是估算:对账 diff = 0

如果是估算,逐天加起来不会精确等于总数。实测精确相等:

```
逐事件之和   = 5,522,401,545
session 总数 = 5,522,401,545     diff = 0   ← 一个 token 都不差
```

代码上这是**构造性保证**:`extractCodexSessionUsage`(session 总数)就是
`extractCodexUsageEvents`(逐事件)的求和——同一段遍历、同一套规则,所以
"逐天之和 == 总数"必然成立,不是凑出来的。
