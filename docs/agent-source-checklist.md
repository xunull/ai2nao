...
| 10 | `web/src/pages/AiRhythm.tsx` | `SOURCE_COLORS` + 图例数组 + `<Area>` **三处分开写**，少一处 tsc 不报 | 该源有没有足够的时间分布密度 |
| 10b | `src/aiRhythm/queries.ts` `WEEK_COUNTED` | 决定「哪条消息算进这张卡」，`total` 必须用它 | 不用它 → `total` 与各带之和脱节，绝对值堆叠图上不会出现空洞、只是矮一截 |
