...
下面 71 条按性质分组，组内不排先后（另有 2 条待归档、5 条本轮销账，列在末尾）。
算式：上一版 65 条活跃 − 5 条销账 + 2 条新增 = 62；2026-08-26 +2、2026-08-27 +1、2026-09-01 +3、2026-09-02 +1 = 69；2026-09-16 +2 = 71。上一版头部写的 67 含了那 2 条待归档。**加粗**的是建议优先看的。
...
- 搜索两条路径的排序规则不一致
- 停更的同步任务审计
- 工具全关时,历史里的 tool_calls 会原样发出去
  *（2026-09-16 工程评审改写：前提仍未证实，销账绑定到「发送视图重建」那一轮）*
...
- **对话账目从消息表搬到正式表** 　*（2026-09-16 加）* —— 上下文/记账那一轮把
  `ChatCall` / `CompactionEvent` / `ReasoningRow` 都塞进 `llm_chat_messages.raw_json`
  （数据结构带 `v: 1` 就是为了以后机械搬迁）。趋势页聚合因此要 `json_extract`，
  而这张表只有 `(session_id, message_index)` 与 `(session_id, role)` 两个索引
  （`migrations.ts:1426-1428`），没有能支撑跨会话按时间/按模型聚合的。
  **前置：同组的「新增迁移会打挂已装的打包版」必须先做**，否则 V61 一加，
  桌面上那个 0.4.0 就开不了库。
...
- 上下文账本抽屉 　*（2026-09-16 加）* —— 把「模型这一轮实际看到了什么」摊开；
  先决条件在上下文/记账那一轮已经做齐，剩下的只有界面（正文见文末）
...
## 旧版打包应用续聊一轮会抹平服务端专有行的保留区下标

What: 0.4.0 打包版的 `replaceLlmChatSessionMessages` 在每轮结束时执行
`UPDATE llm_chat_messages SET message_index = -1000000 - message_index WHERE session_id = ?`,
**没有**新版那条 `AND message_id NOT LIKE 'ai2nao:%'` 守卫,随后按数组位置重新分配下标。
新版写进的账目行(2e6 段)、压缩事件(4e6 段)会被压成普通序号。

Why: 2026-09-18 粗 T13 在库副本上实测(按 0.4.0 bundle 原文复刻那段逻辑,未调模型):
删除行数 0 —— 行都还在(旧版 `agUiMessagesFromSession` 不过滤,`ai2nao:*` 随合并进了提交集合);
但 `ai2nao:call:*` 从 2,000,000 变成 3,`ai2nao:compaction:*` 从 4,000,000 变成 4。旧版随后重读该会话不崩。
这与上面「新增迁移会打挂已装的打包版」是**不同**的危害:不涉及迁移、不报错、纯静默重排。

影响(已查证,不是估计):全仓没有任何地方按 `message_index` 的分段做判定 —— 服务端专有行一律走
`isServerOnlyMessageId` 前缀,`*_ROW_INDEX_BASE` 只用于**分配**。所以不会误判;下一次新版写入仍落在
`BASE + n`,回到普通行之上;压缩事件之间的相对顺序被保留,`replayCompactionStack` 回放仍正确。
损害是「丢了分段」,不是「坏了语义」。

Pros:
- 以后若有人按分段写判定,不会被旧版悄悄坑

Cons:
- 旧版是已发布的二进制,改不了。真正的隔离只有「发布新版时同步替换桌面版」(设计文档《9》已列为硬性前置)

Context: 旧版唯一能触发写回的入口是对话回合(要调模型、要花钱),所以 T13 用复刻而非真实回合。
复刻刻意不 import 当前 `sessions.ts` —— 它带着两道新守卫,用了就等于先把要验的东西修好。

Depends on: 发布新版时替换已装的桌面版

Priority: Phase 2

## 会话记忆(默认开)有两个未查清的卡顿

What: 两个现象,都与会话记忆有关,根因都没查。
1. 一轮里模型调用 `ai2nao_search_session_memory` 时,整个服务失去响应数分钟(主线程卡在同步
   `sqlite3_step` 整表扫描,入口 `src/sessionMemory/service.ts` 的 `search()` 对五个源串行 await)。
2. 2026-09-19 在全新隔离 HOME 里走查:勾着 Memory 发第一条消息,**请求 40 多秒后才发到模型**
   (模型根本没调工具);取消勾选后同样的消息 0.8 秒就开始出字。

Why: 会话记忆是默认勾选的,两个现象都会让「第一印象」直接变成卡死或长时间无响应。

Pros:
- 查清之后可以决定是修慢查询,还是把默认值改成不勾

Cons:
- 第 1 条每次复现要 75 秒 + 杀进程重启,排查成本高

Context: 第 1 条的采样栈与两个 N+1 嫌疑(Cursor `state.vscdb`、Cherry Studio agentsDb)见 AI 对话
上下文设计文档的「探针途中撞到的既有 bug」一节;下一步是按源二分(`sources` 一次只传一个)。
第 2 条只在隔离 HOME 下看到过,没有在真实库上复现过,也可能是首次加载某个索引或模型的一次性开销。
...
What: 聊完一轮带工具调用的对话后，把 `/ai-chat` 的五个工具开关全关掉再发下一条，请求里 `tools` 是 `{}`（`llmTools/registry.ts:31` 起每个工具都是条件挂载），而 `agUiMessagesToModelMessages`（`llmChat/copilotRuntime.ts:705`）会把历史里的 `tool-call` / `tool-result` 原样喂回去。部分厂商对「请求没有 tools 定义、消息却引用了 tool_calls」直接返回 400。

Why: 用户看到的是一句原始厂商报错，完全不知道是自己刚关了个开关导致的。这不是多模型引入的问题，今天就存在；多模型只是把撞上的概率抬高——每轮换家时更容易落到「新的一家 + 旧的一家留下的工具历史」这个组合上。

Pros:
- 修掉一类「报错文案与真实原因完全对不上」的体验坑
- 顺带让每轮切模型这个新功能少一个不确定因素
...
**2026-09-16 工程评审（上下文管理 / 对话记账 / 思考过程）改写了销账路径：**
那一轮要重写 `agUiMessagesToModelMessages`（思考回传、压缩排除、transformTag、工具结果省略），
是距离这条最近的一次改动。决定**不在无证据下压平**，只在发送视图里预留
`transformTag: "tool-flattened"` 这个取值和挂点，不默认启用。
销账条件不变（四家实测），但真要开的时候只剩「把取值接上」这一步。
...
**What:** `/ai-chat` 右侧一个抽屉，把「模型这一轮实际看到了什么」摊开：系统提示、当前生效的
压缩摘要、每条消息的 `transformTag`（原样 / 工具结果被省略 / 图片占位 / 思考回传 / 协议原文回传）、
被排除的消息、以及这一轮每一笔请求的用量与花费。再往上是三个操作：把某条约束**固定**住不被压缩、
**恢复**某段被压缩的原文、**分叉重答**并比较两次的费用。

**Why:** 现在你只能看到 AI 回了什么，看不到它看到了什么。压缩一旦发生，「它是不是忘了某件事」
只能靠猜。这个抽屉把猜变成看，而且发现遗忘时能当场补回去 —— 这是 Codex 在 2026-09-15 冷读这份
设计时说的「最酷的版本」，原话是它最看重 visible and reversible compaction。

**Pros:**
- 数据层已经全部就位，不用再动：`sendView` 指纹（含逐条 `transformTag`）、压缩事件栈、
  逐笔账目、`GET /api/llm-chat/sessions/:id/usage` 的 `context` 分项、分页原文接口
- 是「上下文管理」这条线的自然终点：先让压缩可见可撤销，再让上下文本身可读可编辑
- 对排查「模型为什么答错」极有用 —— 多数时候答案是「它根本没看到那段」

**Cons:**
- 是一个完整的新界面（设计 + 实现 + 测试），CopilotKit 的插槽里放不下，得自己搭
- 「固定约束」「分叉重答」都会反过来要求后端加语义，不只是渲染
- 很容易做成只有作者看得懂的调试面板；真做的时候要重新想清楚给谁看

**Context:** 来自 2026-09-15 `/office-hours` 的 Cross-Model Perspective 一节，
设计文档 `~/.gstack/projects/xunull-ai2nao/quincy-main-design-20260915-091210-ai-chat-context-usage-reasoning.md`
文末「以后再做」也记了一句。2026-09-16 `/plan-eng-review` 决定单独立条，
